...
def pinned_coursier_fetch_dict(
    name,
    maven_install_dict,
    artifacts,
    ):
    """
    Fetch pinned artifacts from a pre-parsed dependency closure dict.

    See rule "pinned_coursier_fetch" for most args.
    ...
    _artifacts = []
    for a in artifacts:
        _artifacts.append(json_parse(a))

    dep_tree = _get_dep_tree(
        name = name,
        json_path = "provided dependency closure",
        maven_install_dict = maven_install_dict,
    )
    _pinned_coursier_fetch_dict_impl(repository_ctx, _artifacts, maven_install_dict)
...
def _pinned_coursier_fetch_impl(repository_ctx):
    ...
    _pinned_coursier_fetch_dict_impl(repository_ctx, artifacts, maven_install_json_content)

def _get_dep_tree(name, json_path, maven_install_dict):
    # Validation steps for maven_install.json.

    # First, validate that we can parse the JSON file.
    if maven_install_dict == None:
        fail("Failed to parse %s. Is this file valid JSON? The file may have been corrupted." % json_path +
             ...
             "  2. Regenerate `maven_install.json` by running the command: bazel run @%s//:pin" % name +
    ...
    if dep_tree_signature == None:
        ...
        fail("%s_install.json contains an invalid signature and may be corrupted. " % name +
...
def _compute_http_files(name, dep_tree):
    ...
    for artifact in dep_tree["dependencies"]:
        if artifact.get("url") != None:
            ...
            http_files.extend([
                ...
                # repository_ctx should point to external/$name
...
def _pinned_coursier_fetch_dict_impl(repository_ctx, artifacts, maven_install_dict, json_path):
    dep_tree = _get_dep_tree(
        name = repository_ctx.name,
        json_path = repository_ctx.path(repository_ctx.attr.maven_install_json),
        maven_install_dict = maven_install_dict,
    )
    http_files = _compute_http_files(name, dep_tree)
...
def _coursier_fetch_impl(repository_ctx):
    ...
    if predefined_maven_install:
        ...
        maven_install_location = "{repository_name}_install"
...
coursier_fetch = repository_rule(
    attrs = {
        ...
        "maven_install_json": attr.label(allow_single_file = True),
        "maven_install_format": attr.string(
            doc = """Defines the
            """,
            default = "json",
            values = [
